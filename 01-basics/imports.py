...
dacas = datetime.now()
print('Aktuální datum a čas:', dacas.strftime('%d.%m.%Y %H:%M:%S'))

# datum velikonoční neděle (easter) v následujících 5 letech
datum = date.today()
year = rrule(freq=YEARLY, count=5, dtstart=datum, bymonth=8, bymonthday=13, byweekday=SU)[0].year
print("Dalsi velikonocni nedele jsou:")
for i in range(0,5):
    rdelta = relativedelta(easter(year), datum)
    year += 1
    print('%s' % (datum+rdelta))

# nejbližší rok, v němž bude Štědrý den v neděli
stedry = rrule(freq=YEARLY, dtstart=datum, bymonth=12, bymonthday=24, byweekday=SU)[0].year
print('Štědrý den v neděli bude roku:', stedry)