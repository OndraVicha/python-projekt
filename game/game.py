...
class Person:
    ...
    def __str__(self):
        ...
        """
        return f'Nickname: {self.nickname}, sex: {self.sex.value}'
...
"""
'''# Vytvoření dvou pokusných objektů hráčů
player1 = Player('Karel', Sex('man'), 'CZE')
player2 = Player('Jana', Sex.female, 'CZE')
# Pozdržení programu do stisku klávesy
input('Počkej chvíli a zadej enter')
# Vytvoření testovacího objektu zápasu mezi výše vytvořenými hráči
match = Match(player1, player2)
# Odehrání virtuálního zápasu
match.play()
# Výpis výsledku zápasu
print(str(match))
# Výpis vývoje skóre zápasu
print(f'Historie zápasu: {match.get_history()}')

# Test načtení hráčů ze souboru players.json
print('-----------------------------------------------------------------')
print('Načtení hráčů ze souboru')
print('-----------------------------------------------------------------')
...
person.append(Player('Jan',Sex.male,'CZE'))
person[2].count_of_games = 5
person[2].wins = 3
person[2].score = {'plus': 24, 'minus': 13}
person[2].score['minus'] += 1
#print(person[0].get_birth(),person[1].get_seconds_from_birth())
print(person[2].win_rate())
players = load_players('players.json')
players[-2].count_of_games = 2
players[-2].wins = 1
print(players[-2].win_rate())
